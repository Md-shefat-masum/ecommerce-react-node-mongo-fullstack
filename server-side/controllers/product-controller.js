...
const { uploadFile } = require("./helper-controller");

const allProduct = async (req, res, next) => {
	let products = await productModel.find().populate("category").populate("creator");

	products = await products.map((product) => {
		let today = new moment();
		let product_discount_time = moment(product.discount_date);
...
async function createProduct(req, res, next) {
	...
	const { title, category, price, discount, discount_price, discount_date, description } = req.body;
...
async function updateProduct(req, res, next) {
	...
	const { _id, title, category, price, discount, discount_price, discount_date, description } = req.body;
